...
class CurrentAssignmentEndpointTests(APITestCase):
    def setUp(self):
        self.biker_user = User.objects.create_user(
            username="current-driver",
            password="Pass@123",
        )
        self.biker = Biker.objects.create(user=self.biker_user)
        ...
        self.url = reverse("current_assignment")

    def test_get_requires_authentication(self):
        response = self.client.get(self.url)
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)

    def test_get_returns_current_active_assignment_for_authenticated_biker(self):
        self.client.force_authenticate(user=self.biker_user)
        response = self.client.get(self.url)

        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertIsNotNone(response.data["assignment"])
        self.assertEqual(response.data["assignment"]["package_id"], self.package.id)
    ...
    def test_get_returns_no_assignment_when_biker_has_no_active_package(self):
        ...
        self.package.biker = None
        self.package.assigned_at = None
        self.package.save(update_fields=["biker", "assigned_at"])
        self.client.force_authenticate(user=self.biker_user)
...
class BikerSalesAndOrdersEndpointTests(APITestCase):
    ...
    def test_order_summary_normalizes_all_driver_statuses(self):
        ...
        expected_by_package_id = {}
        for index, (stored_status, expected_status) in enumerate(status_values.items()):
            package, _ = self.create_order(
                Decimal("20.00") + index,
    ...
    def test_daily_sales_returns_recorded_cash_collections(self):
        ...
        self.assertEqual(
            response.data["sales"][0]["collected_at"], timezone.localdate()
        )
