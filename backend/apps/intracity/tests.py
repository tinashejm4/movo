...
class IntracityPackageListTests(APITestCase):
	def setUp(self):
		...
		PackageStatus.objects.create(package=self.other_package, status="Delivered")
	...
	def test_package_list_returns_only_logged_in_users_packages(self):
		...
		results_by_id = {item["package_id"]: item for item in response.data["results"]}
		self.assertIn(self.own_package.id, results_by_id)
		self.assertIn(self.other_package.id, results_by_id)
		self.assertEqual(results_by_id[self.own_package.id]["status"], "active")
		self.assertEqual(results_by_id[self.other_package.id]["status"], "inactive")
