...
class CustomerRegisterLoginView(APIView):
	...
	def post(self, request):
		...
		print(f"Received request with phone_number: {username} and otp_code: {otp_code}")  # Debugging line
...
class CustomerProfileView(viewsets.ModelViewSet):
	...
	@extend_schema(
		...
		responses={
			...
			404: OpenApiResponse(ErrorResponseSerializer, description="Customer profile not found"),
		},
	)
	...
	def partial_update(self, request, *args, **kwargs):
		...
		user = customer.user
		missing_fields = [field for field in ["first_name", "last_name"] if not getattr(user, field)]
